...
<!--   Elective :
          <select  id="id_elect" name="elect" style="margin-bottom: 10px" >
...
if(isset($_POST['submit']))
{
    ...
    $dept=$_POST["dept"];
		...
		$query = "SELECT sd.sd_id as sd_id,dm.dept_sc as dept, sd.sd_roll as roll , sd.sd_fname as fname,sd.sd_lname as lname FROM student_detail sd, dept_master dm where";
               ...
               if($result->num_rows>0)
               {
                    ...
                    <th width="03%">
                     ...
                     <th width="10%">
							...
							<tbody>';
			$header=array('Sr. No.', 'Dept', 'Roll No.', 'Name', 'CH1', 'CH2', 'CH3', 'CH4', 'CH5', 'CH6');
      $i=1;
      while($row=$result->fetch_assoc())
       {
       	...
       	$p1= array($row["dept"], $row["roll"], $row["fname"].' '.$row["lname"]);
            ...
            $subquery = "SELECT em.elect_shortcode as electch FROM student_elective se,elective_master em WHERE se.elect_id=em.elect_id and se.sd_id=".$row["sd_id"]." and em.dept_id=1 ORDER by se.choice asc";
          ...
          if($subresult->num_rows>0)
          {
            while($subrow=$subresult->fetch_assoc())
            {
                ...
                array_push($p1, $subrow["electch"]);
                $outputtopdfhtml.='<td width="10%">'.$subrow["electch"].'</td>';
            }
          }
          else
          {
                $outputtopdfhtml.='<td width="10%"> </td>
                                <td width="10%"> </td>
                                <td width="10%"> </td>
                                <td width="10%"> </td>
                                <td width="10%"> </td>
