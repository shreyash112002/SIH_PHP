...
$(document).ready(function(){
    ...
    //            // alert(response);
    //             $("#id_elect1").empty();
    //             $("#id_elect1").append("<option value='' disabled selected='selected'>Select an option </option>");
    //             for( var i = 0; i<len; i++){
       ...
       //var semid = $(this).attr("semid");
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                //alert(response);
                $("#id_elect2").empty();
                 $("#id_elect2").append("<option value='' disabled selected='selected'>Select an option </option>");
                for( var i = 0; i<len; i++){
                    var id = response[i]['id'];
                    var name = response[i]['name'];
       ...
       //var semid = $(this).attr("semid");
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                //alert(response);
                $("#id_elect3").empty();
                 $("#id_elect3").append("<option value='' disabled selected='selected'>Select an option </option>");
                for( var i = 0; i<len; i++){
                    var id = response[i]['id'];
                    var name = response[i]['name'];
       ...
       //var semid = $(this).attr("semid");
        ...
        $.ajax({
            ...
            success:function(response){
                ...
                //alert(response);
                $("#id_elect4").empty();
                 $("#id_elect4").append("<option value='' disabled selected='selected'>Select an option </option>");
                for( var i = 0; i<len; i++){
                    var id = response[i]['id'];
                    var name = response[i]['name'];
       ...
       //var semid = $(this).attr("semid");
        ...
        $.ajax({
            ...
            success:function(response){
              ...
              // alert(response);
                $("#id_elect5").empty();
                 $("#id_elect5").append("<option value='' disabled selected='selected'>Select an option </option>");
                for( var i = 0; i<len; i++){
                    var id = response[i]['id'];
                    var name = response[i]['name'];
       ...
       //var semid = $(this).attr("semid");
        ...
        $.ajax({
            ...
            success:function(response){
              ...
              // alert(response);
                $("#id_elect6").empty();
                 $("#id_elect6").append("<option value='' disabled selected='selected'>Select an option </option>");
                for( var i = 0; i<len; i++){
                    var id = response[i]['id'];
                    var name = response[i]['name'];
...
<?php 
  ...
  $stmt = $conn->prepare("SELECT sd_dept,sd_sem,sd_email,sd_contact,isInstSubmitted FROM student_detail where sd_id=? and isDelete=0 and isDisabled=0 ");
...
<div class="panel-heading"> <strong> <p class="text-center">Select Institute Level Elective </p></strong></div>
...
$se_id=1;
 ...
 // echo $se_id;
  ...
  $sql2="Update student_detail set isInstSubmitted=1 where sd_id = '".$sd_id."'";
 // echo $se_id;
  $conn->query($sql2);
...
<div class="panel-heading">  <strong> <p class="text-center ">Your choice for Institute Electives </p> </strong></div>
<div class="panel-body">
...
<table id="video_table" class="table table-bordred table-striped ">
       <tbody>
         ...
         <?php
                ...
                $sql3="SELECT em.elect_name as elect_name FROM student_elective se,elective_master em where em.elect_id=se.elect_id and se.sd_id='".$sd_id."' and em.elect_sem='".$_SESSION['sem_id']."' and em.dept_id='1' and se.isDelete = 0 ORDER BY se.choice ASC";
                ...
                if($result3->num_rows>0)
                {
                  ...
                  $message= 'Dear Student, <br> Your Institute Level Elective Preferences for Sem '.$_SESSION["sem_id"].'  are: <br><br>';
        ?>
                <tr>
                  <td>
                    Sr. No.
        ...
        <tr>
          ...
          $message.='<br> For any discrepancies, do contact Dr. Satishkumar Chavan, Dean of Academics, Don Bosco Insttute of Technology, Mumbai 400070<br>';
        // echo $message;
          sendEmail($emailid,$message);
          echo " <strong> Copy has been sent to registered email </strong> <br>";
        }
        else {
        ?>
        <tr>
