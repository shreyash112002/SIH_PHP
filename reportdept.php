       ...
       // alert(semid);
        $.ajax({
            ...
            success:function(response){
                ...
                var len = response.length;json            LMmmmm 
              // alert(response);
                $("#id_elect").empty();
                 $("#id_elect").append("<option value='notallowed' disabled selected='selected'>Select an option </option>");
                for( var i = 0; i<len; i++){
...
<div class="panel-heading">  <p class="font-weight-italic" style="text-align: center; font-size:24px"> Reports
</p>
...
//  $elect=$_POST["elect"];
    $dept=$_POST["dept"];
		...
		$query = "SELECT sd.sd_id as sd_id, em.elect_name as elect_name, sd.sd_roll as roll , sd.sd_fname as fname,sd.sd_lname as lname FROM student_detail sd,  student_elective se, elective_master em where sd.sd_sem='".$sem."' and sd.sd_dept='".$dept."' and se.choice=1 and se.elect_id =em.elect_id and se.sd_id=sd.sd_id and em.dept_id='".$dept."' and em.elect_sem ='".$sem."' and sd.isDelete=0 and sd.isDisabled=0 ORDER BY se.elect_id asc , sd.sd_roll asc";
               ...
               if($result->num_rows>0)
               {
                    ...
                    <th width="05%">
                       Sr. No.
							...
							<tbody>';
$header=array('Sr. No.', 'Elective', 'Roll No.', 'Name');
$i=1;
while($row=$result->fetch_assoc())
{

$p1= array( $row["elect_name"],$row["roll"], $row["fname"].' '.$row["lname"]);
//print_r($p1);
$outputtopdfhtml.='<tr>
			<td width="05%">'.$i.'</td>
			<td width="25%" style="word-wrap: break-word">'.$row["elect_name"].'</td>
      <td width="15%">'. $row["roll"].' </td>
			<td width="50%" style="word-wrap: break-word">'.$row["fname"].' '.$row["lname"].' </td>';
