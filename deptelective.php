...
<?php 
  ...
  $stmt = $conn->prepare("SELECT isDeptSubmitted FROM student_detail where sd_id=? and isDelete=0 and isDisabled=0 ");
...
//$_SESSION["dept_selected_elect"]=array(0,0,0,0,0,0);
...
<div class="controls col-md-6 ">
  <select class="input-md select form-control" required id="id_elect1" name="elect1" style="margin-bottom: 10px" >
      <option value="" disabled disabled="disabled" selected="selected">Select an option </option>
      
       <?php
      
         $query = "SELECT elect_id,elect_shortcode,elect_name FROM elective_master WHERE dept_id='".$_SESSION['dept_id']."' and elect_sem='".$_SESSION['sem_id']."' and isDelete=0 and isDisabled=0";
...
$value = $_POST['elect1'];
  $sql1="INSERT INTO student_elective(se_id,elect_id,choice,sd_id,dateOfCreation,dateOfDeletion) VALUES('".$se_id."','".$value."','1','".$sd_id."','".$logts."','".$logts."')";
 // echo $se_id;
  $conn->query($sql1);

  $sql2="Update student_detail set isDeptSubmitted=1 where sd_id = '".$sd_id."'";
 // echo $se_id;
  $conn->query($sql2);
  ...
  echo "<br>Elective Recorded added successfully";
        ...
        header('Location: deptelective.php');
        ob_end_flush();
        exit();
...
<table id="video_table" class="table table-bordred table-striped ">
       <tbody>
         ...
         <?php
                ...
                $sql3="SELECT em.elect_name as elect_name FROM student_elective se,elective_master em where em.elect_id=se.elect_id and se.sd_id='".$sd_id."' and em.elect_sem='".$_SESSION['sem_id']."' and em.dept_id='".$_SESSION['dept_id']."' and se.isDelete = 0 ORDER BY se.choice ASC";
                ...
                if($result3->num_rows>0)
                {
                  ...
                  $message= 'Your Department Level elective choices for sem '.$_SESSION["sem_id"].' is : <br><br>';
        ...
        <tr>
          ...
          $message.='<br> For any discrepancies, do contact HOD, Don Bosco Insttute of Technology, Mumbai 400070<br>';
        // echo $message;
